    ...
    run(status, site, app) {
        app.db.query('SELECT * FROM websites WHERE id=?', [site], (err, result) => {
            ...
            app.db.query('SELECT * FROM websites WHERE id=?', [site], (err, result) => {
                ...
                // send to statuspage
                fetch(`https://api.statuspage.io/v1/pages/${app.config.statuspage.page_id}/components/${services.websites[site].component}`, {
                    method: 'patch',
                    body: {
                        component: {
                ...
                const embed = new Discord.RichEmbed()
                    ...
                    .setTitle(`${services.websites[site].name}: ${app.config.statuses[status].title}`)
