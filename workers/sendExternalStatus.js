...
module.exports = {
    ...
    run(status, service, app) {
        ...
        app.db.query(`SELECT * FROM websites WHERE id = "${service}";`, (err, result) => {
            ...
            app.db.query(`UPDATE websites SET status = "${status}" WHERE id = "${service}";`, (err, result) => {
                if (err) return log.error(err);
                log.info(`${services.external[service].name} ${config.statuses[status].info}`)
            });
            ...
            app.db.query(`SELECT * FROM websites WHERE id = "${service}";`, (err, result) => {
                ...
                // send to statuspage
                fetch(`https://api.statuspage.io/v1/pages/${config.statuspage.page_id}/components/${services.external[service].component}`, {
                    method: 'patch',
                    body: JSON.stringify({
                        component: {
                ...
                const embed = new Discord.RichEmbed()
                    ...
                    .setTitle(`${services.external[service].name} (External): ${config.statuses[status].title}`)
                    .setURL(config.statuspage.url)
                    .setDescription(`**${services.external[service].name}** ${config.statuses[status].info}\n\n`)
                    // .addBlankField()
