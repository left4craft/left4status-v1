...
module.exports = {
    ...
    run(app, data) {
        ...
        app.db.query('UPDATE minecraft SET player_count=?, players=?, tps=?, last_online=?, expires=? WHERE id=?;', [data.player_count, data.players.join(','), data.tps, now, expires, data.server], (err, result) => {
