...
module.exports = {
    ...
    run(app, data) {
        ...
        app.db.query(`UPDATE minecraft SET player_count = ${data.player_count}, players = JSON_ARRAY('${data.players}'), tps = ${data.tps}, last_online = ${now}, expires = ${expires} WHERE id = "${data.server}";`, (err, result) => {
            ...
            if (data.tps < config.minecraft.degraded) { // if less than 15
                if (data.tps < config.minecraft.partial)
                    return app.workers.sendServerStatus.run('partial', data, app); // if less than 10
                app.workers.sendServerStatus.run('degraded', data, app); // otherwise if <15 but >10
            } else {
                app.workers.sendServerStatus.run('operational', data, app); // if higher than 15
            };
